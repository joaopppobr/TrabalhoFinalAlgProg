...
void testa_agentes(Coordenada listaagentes[], JOGADOR *jogador, int modo_de_jogo)
{
    int i;

    for(i=0; i<NUM_AGENTES; i++)
    {
        if(listaagentes[i].x == jogador->posicao.x && listaagentes[i].y == jogador->posicao.y)
        {
            jogador->vidas -= 1;
            ...
            putchxy(jogador->posicao.x, jogador->posicao.y, 'P');
            //desenha_placar(nome_jogador, chaves_coletadas, *vidas, tempo_jogo, modo_de_jogo);
        }
    }
}
...
int main()
{
    ...
    do
    {
        ...
        do
        {
            testa_agentes(listaagentes, &Jogador, modo_de_jogo);
